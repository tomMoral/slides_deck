...
%        node[midway, align=center] {\small Problème\\ \small inverse}
    ...
    \hskip3em
    {{\bf Inverse problem:} $\varZ = f(\varX)$}\\
    {
        \vskip1em
...
\frame[t]{
    ...
    \[
        f(\varX) = \varZ^*(\varX) = \argmin_\varZ \frac12\|\varX - \varD\varZ\|_2^2 + \mathcal R(\varZ{})
    \]
    where $\mathcal R$ encodes prior information to select a good/plausible solution.\\[1em]

    \pause{}
    Often solve this optimization problem many times for a given $\varD$,\\[1em]

    \pause{}

    \strongpoint{Can we learn to solve such problem with unrolling?}
    \strongpoint{With convergence guarantees toward the original solution $\varZ^*(\varX)$?}

    \vskip3em

    \emph{$\neq$ setting than supervised learning: $\min_\Theta \mathbb E_{(\varX, \varZ)}\frac12\|\varZ - \Phi_\Theta(\varX)\|_2^2$}
}
...
\frame[t]{
    \frametitle{Prior learning for inverse problem}
    ...
    \[
        \min_{\substack{D,u\\ \|D_k\|_2 \le 1}}\frac12\|\varX-\varD Du\|_2 + \lambda\|u\|_1 \enspace .
    \]
