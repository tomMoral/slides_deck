...
\parttitleframe{Allain2022,Staerman2023}
...
\frame{
    ...
    \vskip2em
    \myitem{} Slow EM algorithm \hskip5ex
    \myitem{} Not general for parametric kernels.
}
...
\frame{
    ...
    {\bf FaDIn} Inference method for general parametric kernels\\[2em]
    \only<1>{
        \centering
        \includegraphics[width=.7\textwidth]{fadin_discretization.png}\\
        {\bf Discretization}\\
    }
    \only<2>{
        \centering
        {\myitem{} \bf Discretization}\\[1em]
        \includegraphics[width=.7\textwidth]{fadin_support.png}\\
        {\bf Finite support}\\
    }
    \only<3>{
        \centering
        {\myitem{} \bf Discretization}\\[1em]
        {\myitem{} \bf Finite support}\\[1em]
        {\myitem{} \bf $\ell_2$ loss}\\
        \[
            \mathcal L(\{t_k\}; \theta) = \sum_{t=0}^T\frac12\| z[t] - (z*k)[t]\|^2_2
        \]
    ...
    %     {\myitem{} \bf Discretization}\\[1em]
    %     {\myitem{} \bf Finite support}\\[1em]
    %     {\myitem{} \bf $\ell_2$ loss}\\[2em]

    %     \begin{columns}
    %         \column{.4\textwidth}
    %             The impact of the discretization can be negligible relative to the statistical error.
    %         \column{.6\textwidth}
    %             \includegraphics[.8\textwidth]{fadin_result_discretization}
    %     \end{columns}
    % }
}
...
\frame{
    \frametitle{Results for evoked atoms - samples}

    {\centering
        \includegraphics[width=\textwidth]{fadin_result_neuro}\\
    }
}
\frame{
    \frametitle{Results for heart rate variability}

    {\centering
        \includegraphics[width=\textwidth]{fadin_result_ecg}\\
    }
}
