...
\begin{document}
    ...
    \frame{
        \frametitle{The MIND Team}

        MIND: Model and Inference for Neural Data\\[2em]

        \myitem{} MRI reconstruction and inverse problems\\[1em]
        \myitem{} Population imaging with neural data\\[1em]
        \myitem{} High Dimensional statistics\\[1em]
        \myitem{} Time-series Analysis for physiological signals\\[1em]
    }
    ...
    \frame{
        ...
        \begin{tikzpicture}[overlay]
            \visible<4->{\draw[<-, thick, shorten >=8, darkblue] (4.1, 1.7) -- +(-1.5, -1.3) node[darkblue] {\emph{Value function}};}
            \visible<3->{\draw[<-, thick, shorten >=35,darkred] (7.3, 1.9) -- +(3, -.2) node {\emph{\color{darkred}Outer function}};}
            \visible<2->{\draw[<-, thick, shorten >=8, lightgreen] (8, .5) -- +(0, -.8) node {\emph{\color{lightgreen} Inner function/Problem}};}
        \end{tikzpicture}
    ...
    \frame{
        ...
        \vskip1em
        {\bf Selecting the best experimental design:}\\[.5em]
        \begin{itemize}\itemsep.7em
            \item $G$ is the reconstruction loss and $\theta$ the reconstructed image.
            \item Select the design $\lambda$ to get the most confident reconstruction evaluation $F$ with the least measurments.\\[1em]
        \end{itemize}

        {\centering
        \includegraphics[width=.4\textwidth]{CT.png}\\
        }
    }
...
\frame{
    ...
    \vskip1em
    \begin{itemize}\itemsep1em
        \item The propose framework allows to adapt many single-level algorithms to the bilevel setting, with similar rates.
        \item \lstinline+Benchopt+ provides a benchmark to quickly test many ideas.
    \end{itemize}

    \strongpoint{Finding Adaptive Optimal Design for CT with bilevel optimization?}

    \vskip2em
    \textbf{Other selected topic of interests}\\[1em]

    \myitem{} Automatic Data-Augmentation,\\[.5em]
    \myitem{} Solving Inverse problem with unrolled algorithms,\\[.5em]
    \myitem{} Unsupervised representation learning for time-series,\\[.5em]
