...
\only<2>{
    ...
    \draw[draw=black, thick, <->, dashed] (southwest.south) -- ++(\width, 0)
        node[midway, below] {Temps $T$};
    \draw[draw=black, thick, <->, dashed] (northeast.east) -- ++(0, -\height)
        node[midway, above, sloped] {Atomes $K$};
}
...
\only<2->{
    ...
    \uncover<2>{
        ...
        -- ++(-\width/\W, 0) node[midway, above, fill=lightblue!70] {Temps $T/W$};
        \draw[draw=black, thick, <->, dashed] (northeast3.east)
        -- ++(0, -\height) node[midway, above, sloped] {Atomes $K$};
    }
}
\only<3->{
    \foreach \w in {1,...,\W-1}{
