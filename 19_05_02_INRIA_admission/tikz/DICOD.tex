...
\only<-2>{
    ...
    \draw[draw=black, thick, <->] (northeast.north) -- ++(-\width, 0) node[midway, above] {T};
...
\only<2->{
    ...
    \only<2>{
        \draw[bold, dashed] ($(southwest.center) + (\w*\widthWorker, 0)$)
                -- ++(0, \height);
              rectangle (northeast\w.center)
       \draw[arrow, ->] ($(southwest.center) + (\w*\widthWorker - \widthWorker / 2, 0)$)
                -- ($(northeast\w) - (\widthWorker/2, 0)$);
    
        \draw[draw=black, thick, <->] (southwest1.south)
            -- ++(\width/\W, 0) node[midway, below] {T / W};
        \draw[draw=black, thick, <->] (southwest1.west)
            -- ++(0, \height) node[midway, left] {K};
    }
