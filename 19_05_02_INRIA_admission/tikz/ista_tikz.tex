...
\draw[linkcolor, thick] ($(h.center) - (.8em,.7em)$)
    -- ($(h.center) - (.2em,0em)$)
    -- ($(h.center) + (.2em,0em)$)
    -- ($(h.center) + (.8em,.7em)$);
\node[right=3.5em of h] (Z) {$Z^*$};
\path (add.east)
edge[pil] (h.west);
\path (h.east)
edge[pil] (Z);
\node[inner sep=0,minimum size=0,right=2.5em of h] (branch) {};
\node[varstyle,below=.5em of h, text width=5em,] (S) {%
    \only<4>{$I - F^\top F / L$}%
    \only<5>{$W_z$}
};
...
\node[below=1em of X] {$\mathcal R(Z) = \|Z\|_1$};
\end{tikzpicture}
\end{document}