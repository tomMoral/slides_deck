...
{\centering \includegraphics[width=.7\textwidth]{blood_cell_images}\\{\footnotesize(Credit to \citealt{Yellin2017})}\\[.5em]}
...
\only<2>{
    ...
    [{\color{linkcolor}\citealt{Dupre2018}, NeurIPS;
    \citealt{Cherkaoui2019}, ICASSP}]\\
    {\usebeamercolor[bg]{normal text}.}\\
}
\only<3>{
...
\begin{itemize}[<+->]
    ...
    \item  Exploring neurological signals from ECG, MEG or fMRI,
...
\begin{frame}[t]{Challenges of Convolutional Dictionary Learning}
    \begin{itemize}\itemsep1em
        ...
        \begin{itemize}
            ...
            \citeconf{Moreau2017}{ICLR}% {\color{linkcolor}, ICLR}]}
            \item {\only<2>{\usebeamercolor[fg]{title}\bf}by parallellization.}\\
            \citeconf{Moreau2018}{ICML};\citeconf{Moreau2019}{preprint}
        \end{itemize}
        \item \textbf{Modelization:} how to incorporate prior knowledge,
        \begin{itemize}
            \item on the activations.\\
             \citeconf{Cherkaoui2019}{ICASSP}
...
\includegraphics[width=\textwidth]{intro_csc}
\only<2>{
    \vskip-2em
    \begin{columns}[T]
...
\frametitle{Notation}
    ...
    \vskip1em
    \begin{itemize}\itemsep1em
        \item $X$ is a signal with $P$ channels on a domain $\Omega$ of size $T$
        \item $\mathcal E$ is a noise signal on the same domain $\Omega$
        \item $\pmb D$ is a set of $K$ patterns with small support $\Theta \subset \Omega$ of size $L$
        \item $Z$ is a signal with $K$ channels on the domain $\Omega$.
    \end{itemize}
\end{frame}
