...
\only<4>{
    ...
    -- ++(0, \height)
    -- cycle;
    \fill[blue!50] ($ (Cm.center) + (\igg*\width /\Lm/2, -\jgg*\height /\K)$)
    -- ++(0, -\height/\K)
    -- ++(\width/2/\Lm, 0)
    -- ++(0, \height/\K)
    -- cycle;
...
\draw[->, thick] ($ (Cm.center) + (\ig*\width /\Lm/2, -\jO*\height /\K) + (\width/\Lm/4, -\height /\K/2)$) arc (135:45:1.1) node [midway, above, fill=blue!10] {$(k, t, \Delta Z)$};
}
\end{tikzpicture}
